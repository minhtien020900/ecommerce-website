...
$categoryList = $CategoryModel->getCategoryListPage($perPage, $page);
$totalRow = $CategoryModel->getTotalCate();

if (isset($_POST['catId'])) {
    if ($CategoryModel->deleteCategoryById($_POST['catId'])) {
        $CategoryModel->deleteBrandCat($_POST['catId']);
        $isDelete = true;
        $active = 'success';
        ...
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item has-treeview menu-open">
              ...
              <ul class="nav nav-treeview">
                <li class="nav-item">
                  <a href="/<?php echo BASE_URL ?>/admin/listcategory.php" class="nav-link active">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
              <div class="search-box">      
                <form class="search-form" action="/<?php echo BASE_URL?>/admin/categorysearch.php" method="get">
                  <input class="form-control" placeholder="Nhập tên danh mục cần tìm" type="text" name="keyword">
...
foreach ($categoryList as $catValue) {
    ?>
            <tr>
              <th scope="row"><?php echo $catValue['category_id']; ?></th>
              <td><?php echo $catValue['category_name']; ?></td>
              <td>
              <?php 
                  $brandList = $CategoryModel->getBrandByCat($catValue['category_id']);
                  $brandValue = explode('/',$brandList['brand_id']);
                  $BrandModel = new BrandModel();
                  foreach ($brandValue as $value) {
                   $brandName = $BrandModel->getBrandtById((int)$value);
                   echo $brandName['brand_name'].' - ';
                  }
              ?></td>
              <td><a href="/<?php echo BASE_URL ?>/admin/addcategory?id=<?php echo $catValue['category_id'] ?>"><i
                    class="fas fa-edit"></i></a></td>              <td>
                <form action="/<?php echo BASE_URL ?>/admin/listcategory.php" method="post"
                  onsubmit="return deleteConfirm()">
                  <input type="hidden" name="catId" value="<?php echo $catValue['category_id'] ?>">
        ...
        <?php
echo $pagination->createPageLinks('listcategory.php', $totalRow, $perPage, $page);
?>
    </div>
  </div>
  <!-- /.card-body -->
